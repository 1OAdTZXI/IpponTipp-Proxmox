...
4. Deploy manually from exact tags. RC accepts
   ...
   channel.
5. Build locked dependencies and static assets in a new release directory,
   apply migrations, switch the active code symlink, restart Gunicorn and
   Celery services, and verify the database-aware health endpoint.
...
- A failed health check can restore the previous code symlink, but applied
...
- Script syntax and release selection are tested, but template discovery,
  storage, networking, package installation, and service startup still require
  an end-to-end acceptance run on the target Proxmox host.
